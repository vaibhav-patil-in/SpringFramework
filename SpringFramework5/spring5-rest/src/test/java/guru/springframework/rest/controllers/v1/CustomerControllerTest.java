...
public class CustomerControllerTest extends AbstractRestControllerTest {
    ...
    public void testGetCustomerById() throws Exception {
        ...
        //when
        mockMvc.perform(get(getCustomersUrl(1L))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    public void testUpdateCustomer() throws Exception {
        ...
        returnDTO.setCustomerUrl(getCustomersUrl(1L));

        when(customerService.saveCustomerByDTO(anyLong(), any(CustomerDTO.class))).thenReturn(returnDTO);

        //when/then
        mockMvc.perform(put(getCustomersUrl(1L))
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(customer)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.firstName", equalTo("Fred")))
                .andExpect(jsonPath("$.lastName", equalTo("Flintstone")))
                .andExpect(jsonPath("$.customerUrl", equalTo(getCustomersUrl(1L))));
    }    
    ...
    public void testPatchCustomer() throws Exception {
        ...
        mockMvc.perform(patch(getCustomersUrl(1L))
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(customer)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.firstName", equalTo("Fred")))
    ...
    public void testDeleteCustomer() throws Exception {
        ...
        mockMvc.perform(delete(getCustomersUrl(1L))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    ...
    private String getCustomersUrl(Long id) {
    	return new StringBuffer(CustomerController.BASE_URL).append("/").append(id.longValue()).toString();
    }
